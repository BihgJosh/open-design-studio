...
const readProjects = () => JSON.parse(fs.readFileSync(projectFile, 'utf8'));
const saveProjects = p => fs.writeFileSync(projectFile, JSON.stringify(p, null, 2));
...
const defaultProject = () => ({ id: crypto.randomUUID(), name: 'Untitled design', brief: '', width: 390, height: 844, background: '#f5f7fb', elements: [
...
function validateProject(p) {
  if (!p || typeof p !== 'object' || typeof p.id !== 'string' || typeof p.name !== 'string' || !Array.isArray(p.elements)) throw Error('Invalid design');
  if (p.elements.length > 250 || JSON.stringify(p).length > 2_000_000) throw Error('Design is too large');
  return p;
}
function handoffHtml(p) {
  const items = p.elements.map(e => `<div style="position:absolute;left:${Number(e.x)||0}px;top:${Number(e.y)||0}px;width:${Number(e.w)||100}px;height:${Number(e.h)||40}px;box-sizing:border-box;overflow:hidden;display:${e.type==='button'?'flex':'block'};align-items:center;justify-content:center;padding:${e.type==='button'?'0 12px':'0'};font:${escapeHtml(e.weight||'400')} ${Number(e.fontSize)||16}px Arial,sans-serif;color:${escapeHtml(e.color||'#111827')};background:${escapeHtml(e.background||'transparent')};border-radius:${Number(e.radius)||0}px;white-space:pre-wrap">${escapeHtml(e.text||'')}</div>`).join('\n');
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(p.name)}</title><style>body{margin:0;background:#e9edf4;display:grid;place-items:center;min-height:100vh}.screen{position:relative;width:${Number(p.width)||390}px;height:${Number(p.height)||844}px;background:${escapeHtml(p.background||'#fff')};box-shadow:0 20px 70px #17213933;overflow:hidden}</style><div class="screen">${items}</div></html>`;
}
function handoffBrief(p, version) {
  return `# ${p.name}\n\nApproved design version: ${version}\nExported: ${new Date().toISOString()}\n\n## Product brief\n${p.brief || 'No brief supplied.'}\n\n## Implementation instructions\nImplement the approved design represented in design.json and preview.html. Preserve content, hierarchy, colors, spacing, and responsive intent. Use the existing project stack and components where possible. Compare the implemented screen with preview.html at ${p.width} × ${p.height} before calling it complete. Ask about unspecified behavior instead of inventing business rules.\n\n## Elements\n${p.elements.map(e => `- ${e.type}: “${String(e.text||'').replace(/\n/g,' ')}” at (${e.x}, ${e.y}), ${e.w} × ${e.h}`).join('\n')}\n`;
}
  ...
  try {
    ...
    if (u.pathname.startsWith('/api/')) {
      ...
      if (req.method === 'POST' && u.pathname.match(/^\/api\/projects\/[^/]+\/handoff$/)) {
        ...
        const version = approved.label; const design = {...p, ...approved.snapshot, approvedVersion: version}; const folder = path.join(handoffDir, clean(p.name) + '-' + clean(id)); fs.mkdirSync(folder, {recursive:true});
        ...
        return send(res, 200, {folder, prompt:`Implement the approved design in ${folder} using IMPLEMENT.md, design.json, and preview.html. Compare the result with the preview before finishing.`});
      }
